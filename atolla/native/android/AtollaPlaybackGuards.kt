...
object AtollaPlaybackGuards {
	...
	// hold the gapless next item / lookahead top-up back until the current item reaches
	// STATE_READY, so building its decoder doesn't compete with the current track starting
	...
	// requested one (window indices; -1 = unknown, disables suppression). mismatch rebuilds
	// only, never when ended/idle, which need a rebuild to re-prepare. allowBackwardRebuild is
	// the caller's intent: a deliberate previous/back-to passes true and is honored
	...
	// later mis-classify an unrelated SEEK as an engine advance/step-back
