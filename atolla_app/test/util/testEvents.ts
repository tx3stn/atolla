...
// under test only read a few fields, so we synthesise minimal values (casting past the unused
// required fields).
...
// (View | Layout) props, so colour/opacity aren't reachable on the union without narrowing;
// tests only assert their values.
